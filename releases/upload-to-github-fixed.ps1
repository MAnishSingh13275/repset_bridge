...
if (-not (Test-Path $zipFile)) {
    ...
    Write-Host "Please ensure GymDoorBridge-v1.1.0.zip exists in the current directory" -ForegroundColor Yellow
    exit 1
}

if (-not (Test-Path $releaseNotes)) {
    Write-Host "Release notes not found: $releaseNotes" -ForegroundColor Yellow
