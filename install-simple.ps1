# ================================================================
# RepSet Bridge - Complete Installation Script (Simple Version)
...
try {
        ...
        try {
            ...
            if (Test-Path $DownloadZip) {
                ...
                if ($fileSize -gt 1MB) {
                    ...
                    Write-Log "[OK] Download successful ($sizeMB MB)" "Success"
                    break
                }
            }
        } catch {
    ...
    Write-Log "[OK] Found bridge executable: $($bridgeExe.Name)" "Success"
    ...
    try {
        ...
        Write-Log "[OK] Bridge installed: $version" "Success"
    } catch {
        Write-Log "[OK] Bridge installed to $InstallPath" "Success"
    }
...
"@
    ...
    Write-Log "[OK] Configuration created" "Success"
    ...
    try {
        ...
        Write-Log "[OK] Windows Defender exclusions added" "Success"
    } catch {
            ...
            switch ($method) {
                "BuiltIn" {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        $serviceCreated = $true
                        Write-Log "[OK] Service created using bridge installer" "Success"
                        break
                    }
                }
                "PowerShell" {
                    ...
                    $serviceCreated = $true
                    Write-Log "[OK] Service created using PowerShell" "Success"
                    break
                }
                "sc.exe" {
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        $serviceCreated = $true
                        Write-Log "[OK] Service created using sc.exe" "Success"
                        break
                    }
                }
            }
    ...
    if ($PairCode) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                Write-Log "[OK] Device pairing successful!" "Success"
                
                # Extract device ID from result
                $deviceIdPattern = "device_id([a-zA-Z0-9_]+)"
                $deviceIdMatch = $pairResult | Select-String $deviceIdPattern
                if ($deviceIdMatch) {
                    $deviceId = $deviceIdMatch.Matches[0].Groups[1].Value
                    Write-Log "[OK] Device ID: $deviceId" "Success"
                }
            } else {
                $pairResultString = $pairResult -join " "
                if ($pairResultString -match "network|connection|timeout|server") {
                    Write-Log "[WARN] Pairing delayed due to network - will retry automatically" "Warning"
                } else {
                    Write-Log "[WARN] Pairing had issues: $pairResultString" "Warning"
                }
            }
        } catch {
            Write-Log "[WARN] Pairing error: $($_.Exception.Message)" "Warning"
        }
    } else {
    ...
    try {
        ...
        if ($service.Status -eq "Running") {
            Write-Log "[OK] Service started successfully!" "Success"
            
            # Quick health check
            Start-Sleep 3
            try {
                $healthCheck = Invoke-WebRequest -Uri "http://localhost:8081/health" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop
                Write-Log "[OK] Bridge API responding (HTTP $($healthCheck.StatusCode))" "Success"
            } catch {
                Write-Log "[WARN] Bridge API not yet available (normal during startup)" "Warning"
            }
        } else {
            Write-Log "[WARN] Service status: $($service.Status)" "Warning"
        }
    } catch {
        Write-Log "[WARN] Service start failed: $($_.Exception.Message)" "Warning"
