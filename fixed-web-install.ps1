...
try {
    ...
    try {
        ...
        # Start service (attempt but don't fail on timeout)
        Write-Step "7/8" "Starting service..."
        Write-Info "Attempting to start service..."
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Info "Service did not start immediately (exit code: $LASTEXITCODE)"
                Write-Info "Output: $startResult"
                $serviceStarted = $false
            }
        } catch {
            Write-Info "Service start command failed: $($_.Exception.Message)"
            $serviceStarted = $false
        }
        Write-Host ""
        ...
        if ($service) {
            ...
            if (-not $serviceStarted) {
                Write-Info "Note: Service startup will succeed after device pairing"
            }
        } else {
    ...
    if ($PairCode) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host ""
                if ($pairResult -match "already paired") {
                    Write-Warning "Device is already paired with the platform"
                    Write-Info "Installation completed successfully - device is ready to use"
                    Write-Info "To use a new pairing code, first run: gym-door-bridge unpair"
                } else {
                    Write-Warning "Pairing failed. You can try again later."
                    Write-Info "Pairing output: $pairResult"
                    Write-Info "Installation was successful - you can pair manually later"
                }
            }
        }
        finally {
    ...
    Write-Host ""
    Write-Success "Web installation completed successfully!"
    Write-Info "The Gym Door Bridge service is installed and ready to use"
    if (-not $Silent) {
